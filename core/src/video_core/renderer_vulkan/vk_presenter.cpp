...
void Presenter::Present(Frame* frame, bool is_reusing_frame) {
    ...
    // Z4.52E: disable legacy hot-path trace logging during performance testing.
    ...
    bool bachata_acquired = swapchain.AcquireNextImage();

    if (!bachata_acquired) {
        swapchain.Recreate(window.GetWidth(), window.GetHeight());

        bachata_acquired = swapchain.AcquireNextImage();
