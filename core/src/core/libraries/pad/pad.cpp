...
// Some titles poll scePadReadState with handle 1 without first calling scePadOpen.
// Preserve Zenith's known-working auto-open/alias behavior while using the
// newer upstream controller State implementation.
static s32 EnsurePadHandle(s32 handle) {
    ...
    s32 opened = existing != pad_handle_map.end()
                     ? existing->second
