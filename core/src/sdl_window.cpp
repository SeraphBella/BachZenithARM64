...
WindowSDL::WindowSDL(s32 width_, s32 height_, Input::GameControllers* controllers_,
    ...
    if (!SDL_InitSubSystem(SDL_INIT_GAMEPAD)) {
        LOG_ERROR(Input, "Failed to initialize SDL gamepad subsystem: {}", SDL_GetError());
    } else {
        LOG_INFO(Input, "SDL gamepad subsystem initialized");
    }
...
void WindowSDL::WaitEvent() {
    ...
    static int z441_probe_count = 0;
    if (z441_probe_count < 20) {
        int z441_gamepad_count = -1;
        SDL_JoystickID* z441_ids = SDL_GetGamepads(&z441_gamepad_count);
        LOG_INFO(Input, "Z4.41 POST-WAIT event={} gamepads={} error='{}'",
                 event.type, z441_gamepad_count, SDL_GetError());
        SDL_free(z441_ids);
        ++z441_probe_count;
    }
