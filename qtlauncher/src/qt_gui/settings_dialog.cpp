...
void LogUpdateLevels() {
    ...
    if (EmulatorSettings.IsLogEnable()) {
        for (const auto class_level : std::views::split(EmulatorSettings.GetLogFilter(), ' ')) {
            const std::string class_level_string(class_level.begin(), class_level.end());
            const auto separator = class_level_string.find(':');

            if (separator == std::string::npos ||
                class_level_string.find(':', separator + 1) != std::string::npos) {
                std::cerr << "bad log filter provided" << std::endl;
                continue;
            }

            const std::string class_name = class_level_string.substr(0, separator);
            const std::string level_name = class_level_string.substr(separator + 1);

            if (!class_name.empty() && class_name.front() == '*') {
                default_log_level = spdlog::level_from_str(level_name);
            } else {
                log_level_per_class[class_name] = spdlog::level_from_str(level_name);
            }
        }
    }
...
SettingsDialog::SettingsDialog(std::shared_ptr<gui_settings> gui_settings,
    ...
    // Experimental
    {
        connect(ui->shaderCaheCheckBox, &QCheckBox::stateChanged, this,
                [this](int state) {
    ...
    if (EmulatorState::GetInstance()->IsGameRunning()) {
        ...
        connect(ui->FSRCheckBox, &QCheckBox::stateChanged, this, [this](int state) {
            if (is_game_specific == EmulatorState::GetInstance()->IsGameSpecifigConfigUsed()) {
        ...
        connect(
            ui->RCASCheckBox, &QCheckBox::stateChanged, this, [this](int state) {
                if (is_game_specific == EmulatorState::GetInstance()->IsGameSpecifigConfigUsed()) {
...
void SettingsDialog::LoadValuesFromConfig() {
    ...
    ui->logMaxSkipDurationLineEdit->setValue(EmulatorSettings.GetLogMaxSkipDuration());
    ui->logSeparateCheckBox->setChecked(EmulatorSettings.IsLogSeparate());
    ui->logSizeLimitLineEdit->setValue(EmulatorSettings.GetLogSizeLimit());
